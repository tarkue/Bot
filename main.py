...
async def start(event: SimpleBotEvent) -> str:
    return await event.answer(
        "Привет! Это текст начать!", 
        keyboard=KSTART.get_keyboard()
    )

@bot.message_handler(bot.payload_filter(payload={"command":"help"}))
async def help(event: SimpleBotEvent) -> str:
    await fsm.set_state(
        event=event, state=Question.wait, for_what=ForWhat.FOR_USER
    )
    await fsm.add_data(
        event=event,
    ...
    await event.answer(
        """
        Опишите вашу проблему в деталях, мы обработаем ваше обращение в ближайший обход!
